import { SyncPricesCommand } from '../SyncPricesCommand';
...
export class SyncPricesCommandHandler {
  async handle(command: SyncPricesCommand): Promise<void> {
    logger.info(`Starting prices sync for loja ${command.lojavirtual_id}`, {
    ...
    try {
      ...
      // 4. Get prices from ProdutoTabelaPreco
      // 5. Sync via adapter

      logger.info(`Prices sync completed for loja ${command.lojavirtual_id}`);
    } catch (error: any) {
      logger.error('Error during prices sync', {
        lojavirtual_id: command.lojavirtual_id,
        error: error.message,
        stack: error.stack,
      });
